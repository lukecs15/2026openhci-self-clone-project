...
三個情境的影像/文案/立場全部在 `src/data/scenarios.js`。影像優先序：
`video`（`public/scenarios/scenario-N.mp4`，導入頁展示 + 辯論頁半透明
全螢幕背景）→ `image`（`scenario-N.jpg`）→ `fallbackImage`（佔位 SVG）。
mp4/jpg 放進 `public/scenarios/` 即自動生效，載入失敗自動逐級退回，
不需要改程式。立場球的骨架樣式（E/A/C/N/O）與顏色（hue）也在同一個
檔案設定。
