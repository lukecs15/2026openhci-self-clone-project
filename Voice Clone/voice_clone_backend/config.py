...
class Settings(BaseSettings):
    ...
    # 補一段等待。**預設已停用（0）**：前端（網頁版與 VR 版皆是）現在都會
    # 用 turn_played 真實回報「這一輪播完了」（見 routers/ws_debate.py），
    # 節奏由真實播放進度決定，這段估計值等待變成同一件事等兩次的重複延遲，
    # 也會拖慢「預生成下一輪」的投機生成（投機輪會先睡滿估計時長才進
    # buffer）。如果之後有不回報 turn_played 的前端要接進來，再把這個值
    # 調回正數（例如 12.0）即可，估算邏輯都保留在 agents/debate.py。
    debate_max_pacing_seconds: float = 0.0
