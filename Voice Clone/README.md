...
116 個測試，涵蓋範圍：STT 主備切換（逾時/例外/正常，含「primary 與 fallback
...
查詢/刪除、自動轉錄逐字稿、以及 TTS 服務正確查到指派給 agent 的克隆聲音
profile、**結束對話總結**（`generate_summary()`：正確把歷史格式化並用
`agent_id="summary"` 呼叫 LLM、空歷史時的保底句子、辯論模式 system prompt
帶入主題標題；`MockLLMService` 依 `agent_id` 區分一般回覆與總結回覆；
`ConversationSession` / `DebateSession` 的 `generate_summary()` 薄封裝
委派邏輯，見第十節）。
...
135 個測試，涵蓋範圍：`agentSessionReducer` / `debateSessionReducer` 狀態機
（WebSocket 事件如何驅動 UI 狀態，含多 agent 同時發話情境、暫停/插話狀態轉換、
「chunk 文字為空不重複記錄 transcript」的迴歸測試、`session_summary` 事件
的狀態轉換／`disconnected` 不會蓋掉 `summary` 狀態／`reset` 還原初始狀態，
見第十節）、`applyVoiceProfileToAgents`
...
`mergeWaveformSignatures`（線性欄位算術平均、hue 圓形平均含跨 0/360
邊界案例、空陣列/null 的保底簽章、單一簽章結果約等於原簽章、缺
colorIntensity 欄位防呆，見第十節）。
...
Server → Client:
  ...
  session_summary      { text }   # end_session 收到後、關閉連線前送出的總結紀念語，見第十節
  error                { message }
```
...
4. 開始討論 → `components/DebateStage.jsx`：兩位 agent 交替發言，畫面右下角
   ...
   變明顯）。點擊後**不會**立刻回到選主題畫面，而是送出 `end_session`
   等待後端生成總結，切換成全螢幕結束畫面（見第十節）——一般多 Agent 對話
   （`VoiceAgentsPage.jsx` 的 chat 分支）也比照同一顆按鈕的視覺／互動設計，
   新增了對應的「結束對話」按鈕
...
Client → Server:
  init_debate_session { topic_id, agents }   # agents 恰好 2 位，第一位先開口
                       # topic_id="custom" 時可帶 topic_title 自訂議題（見第十二節：
                       # 手機 onboarding 傳來的使用者煩惱，後端 build_custom_topic()
                       # 動態組出 DebateTopic，不需要存在 DEFAULT_DEBATE_TOPICS）
...
Server → Client:
  ...
  session_summary        { text, verdict }   # end_session 收到後、關閉連線前送出；text 是
                       # 結語一句話（向後相容舊前端），verdict 是「內在法庭」結構化
                       # 判決書（見第十二節），判決書生成失敗時 fallback 成只有 text
  error                  { message }
```
...
## 十、結束對話／討論的總結紀念語與融合波形

需求：多 Agent 對話模式與辯論模式，使用者按下結束按鈕後，把對話紀錄
（辯論模式含辯論主題）交給 LLM 生成一句勵志／心靈雞湯／鼓勵意涵的總結，
作為使用者結束體驗後可以帶走的紀念品；同時把所有參與過對話的 agent 波形
「融合」成一個最終波形，視覺化呈現體驗結束後的結果。目前只在網頁端呈現，
把總結句子與波形傳送到使用者手機是之後的目標（見第十一節）。

### 10.1 後端：`generate_summary()`（`agents/orchestrator.py` / `agents/debate.py`）

兩個 orchestrator 都新增 `generate_summary()`：把 `self.history` 整理成
「顯示名稱／使用者：內容」逐行文字（辯論模式額外把 `topic.title` 帶進
system prompt），塞進單一個 user message，固定用 `agent_id="summary"`
呼叫 `llm_service.stream_reply()`——刻意不重用既有的 `_build_messages()`
（那是「幫某個 agent 產生下一句回覆」用的格式，語意不同：這裡要「把整段
歷史當材料，請 LLM 用旁觀者視角整理感想」）。`agent_id="summary"` 不對應
任何一位 agent，`MockLLMService` 也用同一個 id 判斷要回傳「總結用」的
固定腳本（`summary_reply` 建構子參數，預設一句鼓勵語），而不是一般對話的
`scripted_reply`，讓沒接真實 LLM 的開發環境也能看到有意義的結束總結。
`pipeline/conversation_pipeline.py` 的 `ConversationSession` 與
`pipeline/debate_pipeline.py` 的 `DebateSession` 都只是薄封裝，委派給
`orchestrator.generate_summary()`。

`routers/ws_voice_agents.py` / `routers/ws_debate.py` 的 `end_session`
處理都改成：先產生總結、送出 `session_summary` 事件，才真正 `break`
出主迴圈斷線（辯論模式必須先 `_cancel_debate_task()` 停掉背景生成 task，
理由跟 `pause_debate` 一樣：避免它繼續往 `event_queue` 塞下一輪事件跟
`session_summary` 搶時序）。生成失敗（例如 LLM API 逾時/出錯）只記
log、不阻塞斷線流程，避免使用者卡在「按了結束卻沒反應」。

`models/schemas.py` 的 `ServerMessageType` / `DebateServerMessageType`
都新增 `"session_summary"`，沿用既有的 `text` 欄位承載總結句子，不需要
新增欄位。

### 10.2 前端：融合波形怎麼算（`utils/waveformSignature.js` 的 `mergeWaveformSignatures()`）

確認的設計方案：**平均簽章＋套用總結句子的情緒**——重用既有架構，不是
另外寫一套情緒疊加邏輯：

1. `mergeWaveformSignatures(signatures)` 把所有參與過對話的 agent 波形
   簽章平均成一個共同基準：`frequency`／`amplitude`／`waveHeight`／
   `waveformShape`／`colorIntensity` 都是線性數值，直接算術平均；`hue`
   是角度（0～360 循環），算術平均會出錯（例如 350 度跟 10 度的算術
   平均是 180 度——完全相反的顏色，正確答案是 0 度），改用「取各角度的
   單位向量、平均後再轉回角度」的標準圓形平均（circular mean）算法。
2. 把這個平均後的簽章，跟總結句子一起丟進**既有的** `WaveformAvatar`：
   `signature` prop 傳融合後的基準簽章、`currentText` prop 傳總結句子，
   `WaveformAvatar` 內部本來就有 `applyEmotionSignal(signature,
   analyzeTurnEmotion(currentText))` 疊加情緒、`lerpSignatureTowards()`
   平滑過渡的邏輯（見第九節），完全不需要另外寫一套。
3. 過渡效果：融合波形本身用 `lerpSignatureTowards()` 平滑過渡；使用者
   後續進一步要求要有「Agent 波形融合」的視覺化動畫，因此另外疊加了
   一層「各 agent 波形飛向中心再融合」的匯聚動畫（見 10.3 節的
   `AgentMergeConverge`），兩者分工不同：這裡的平滑過渡負責融合波形
   本身的情緒表現，匯聚動畫負責「視覺敘事」。

### 10.3 前端：全螢幕結束畫面（`components/SessionSummaryScreen.jsx`）

呈現方式採用**全螢幕沉浸式結束畫面**（對照 app 整體的沉浸式體驗設計
方向）：蓋住整個對話 UI，只顯示融合波形（鋪滿背景，直接重用
`WaveformAvatar` 滿版波場的呈現方式）與總結句子，按下畫面上的離開按鈕
才會回到起始畫面。

狀態流轉：`agentSessionReducer.js` / `debateSessionReducer.js` 都新增
`session_summary` action，把 `status` 轉成 `'summary'`、記錄
`summaryText`；`case 'disconnected'` 也都加了防呆——如果當下 `status`
已經是 `'summary'`，緊接在 `session_summary` 之後的 WebSocket 斷線事件
不會把狀態蓋回 `'idle'`，讓使用者穩定停留在結束畫面。`useVoiceAgentSession.js`
/ `useDebateSession.js` 都新增 `reset()`（送出 `{type:'reset'}`
action），在 `VoiceAgentsPage.jsx` 的「離開結束畫面」處理函式裡，先
`disconnect()` 再 `reset()`，避免下一次重新開始對話/討論時畫面殘留
上一場的 transcript／summaryText／status。

按下「結束對話／討論」按鈕時**刻意不立刻斷線**（只送出 `end_session`）：
提早斷線會讓前端永遠收不到後端送回來的 `session_summary`（後端在
WebSocket 斷線前才會送出這個事件，見 10.1 節）。

**進場轉場**（使用者實測回報第一版直接硬切到結束畫面太生硬）：現在的
進場分三個依序、不重疊的階段——

1. 掛載後用一個 `visible` state（透過 `requestAnimationFrame` 延後一幀
   才切成 true，確保瀏覽器先畫出 opacity:0 的初始樣式一次，樣式變化才
   會真的觸發 CSS transition，而不是「初始值跟目標值同一幀套用」導致
   沒有轉場效果）讓整個畫面（背景＋融合波形）柔和淡入。
2. 淡入同時播放「Agent 波形融合」匯聚動畫（見下方），播完才進入第 3 階段。
3. 總結文字跟離開按鈕依序延遲淡入＋些微上移。

**Agent 波形融合的匯聚動畫**（`components/AgentMergeConverge.jsx`，
使用者明確要求的視覺化效果）：第一版是幾張方形卡片（裡面塞一個完整的
`WaveformAvatar`）直線縮小消失，使用者實測回報「有點糟糕」、希望有更
高沉浸感的品質，因此整個重做成「發光能量球」設計：

- 每一位參與過對話的 agent，用 `getWaveformSignature(agent)` 算出的
  自己原本的波形人格簽章決定顏色，畫成一顆用 `radial-gradient` +
  `box-shadow` 營造柔和光暈的圓形「能量球」（不是方框卡片），球心疊一小段
  用 `utils/waveformPath.js` 的 `buildWavePath()` 算出的靜態波形線條
  （顏色來自 `utils/waveformColor.js` 的 `buildWaveformColors().glow`），
  保留「這是一個波形」的視覺識別，但只算一次靜態 path，不會在每顆球裡都
  額外跑一份完整的 `WaveformAvatar` 動畫迴圈。
- 每顆球用 `mixBlendMode: 'screen'` 跟底下已經在播放的融合波形做加法
  混色，光暈感覺像同一個光源系統的一部分，而不是浮在上面的不透明 UI
  元素。
- 移動路徑不是直線：先在一個橢圓環上分布（依索引平均分布角度，半徑用
  `vw`/`vh` 單位，天生 responsive、不需要 `ResizeObserver`），用 CSS
  `@keyframes` 搭配一個「中繼點」（起點繞中心旋轉固定角度、往內收）讓
  所有球一起朝同一方向旋轉內縮，形成類似漩渦被吸入中心的動態，比單純
  直線收斂更有整體感與流動感。
- 每個 agent 除了本體球，額外疊 2 顆更小、更透明、更模糊的「殘影」球
  （沿同一條路徑、只是起跑時間稍微晚一點），營造柔和拖尾光跡。
- 收斂完成的瞬間，畫面中心會有一個短暫的白色光暈「綻放」
  （`agentMergeBurst` 這組 keyframes），呼應「所有波形真的融合在一起
  了」的敘事高潮，而不是球體悄悄消失、什麼反應都沒有。

**技術作法**：改用 CSS `@keyframes` 動畫（而不是 `transition`）——
`animation` 屬性一旦掛上就會自動從 `0%` 關鍵影格開始播放，搭配
`animation-fill-mode: both` 讓 `animation-delay` 期間也維持在 `0%`
關鍵影格的樣子，不需要像 `SessionSummaryScreen.jsx` 的淡入效果那樣，
用「先掛載初始樣式、下一幀才切換」的雙緩衝手法（那是給 CSS
`transition` 用的，`animation` 不需要）。CSS 自訂屬性（`--start-x` /
`--start-y` / `--mid-x` / `--mid-y`）帶入每個球各自不同的起點/中繼點
座標，所有球共用同一組 `@keyframes` 定義。

**設計取捨**：沒有做「真的從對話畫面裡每個 agent 方框當時的實際像素
位置飛過來」的 FLIP 動畫——結束畫面是獨立掛載的全螢幕 overlay，量測不
到「上一刻」AgentStage 方框的真實位置，硬做容易對不準、感覺卡卡的，
改用環狀分布、集體同向旋轉內縮的簡化版本，敘事意圖一樣成立、實作複雜度
低很多。動畫時間跑完（含每個 agent 的 stagger 延遲 + 緩衝時間）會呼叫
`onComplete`，`SessionSummaryScreen` 才把 `mergePhase` 切成 `'done'`，
開始淡入文字。會尊重 `prefers-reduced-motion`：使用者系統設定開啟
「減少動態效果」時直接跳過匯聚動畫、總結文字改成緊接在畫面淡入之後就
出現。

### 10.4 已涵蓋的測試

後端（pytest）：`generate_summary()` 正確把歷史格式化並用
`agent_id="summary"` 呼叫 LLM、空歷史時的保底句子、辯論模式的
system prompt 有帶入主題標題、`MockLLMService` 依 `agent_id` 區分
一般回覆與總結回覆、`ConversationSession` / `DebateSession` 的薄封裝
委派邏輯。

前端（vitest）：`mergeWaveformSignatures()` 的線性欄位算術平均、hue
圓形平均（含跨 0/360 邊界與三點均勻分布的邊界案例）、空陣列/null 的
保底簽章、單一簽章時結果約等於原簽章、缺 `colorIntensity` 欄位的防呆；
兩個 reducer 的 `session_summary` 狀態轉換、`disconnected` 在
`summary` 狀態下不會被蓋掉、`reset` 還原成初始狀態。

> 注意：`SessionSummaryScreen.jsx` / `AgentMergeConverge.jsx` 本身（轉場
> 時間軸、匯聚動畫的 CSS transition）沒有對應的 vitest 測試，沿用專案
> 一貫的測試慣例（見第六節開頭）：只有抽成純函式、跟 React/DOM 無關的
> 邏輯（`mergeWaveformSignatures()` 等）才寫單元測試，React 元件的渲染
> /動畫效果本身不寫測試（沒有引入 React Testing Library），需要用瀏覽器
> 肉眼驗證轉場節奏是否流暢。

## 十一、尚待整合（對照原架構文件「尚待討論」章節）
...
8. ~~辯論模式主題目前是後端寫死的三個選項（`agents/debate.py` 的
   ...
   REST/WS 介面與前端輸入欄位。~~ 已完成：`init_debate_session` 支援
   `topic_id="custom"` + `topic_title`（`build_custom_topic()` 動態組出
   DebateTopic）；議題來源是手機 onboarding 的 `topic` 欄位（使用者輸入/
   選擇，或留空由後端從聲音樣本逐字稿推導，見 `services/topic_derivation.py`
   與第十二節）。網頁版前端的固定三選一選單仍可照舊使用，不受影響。
...
10. ~~結束對話／討論的總結句子與融合波形（見第十節）目前只在網頁端呈現；
    最終理想上會把生成的總結句子與波形傳送到使用者的手機，作為體驗的
    紀念品——這個「傳送到手機」的部分還沒有設計對應的傳輸方式（例如
    QR code 帶一個一次性連結、還是另外接推播），是刻意先不做的下一步。~~
    已完成：走「QR code 帶一次性連結」方案——主系統（Unity）結束時把
    判決書 POST 回 `/api/onboarding-sessions/{id}/result`，並顯示手機
    結果頁網址的 QR（`GET /api/qr` 產生），手機掃碼後 `ResultPage.jsx`
    輪詢 `/result` 取得判決書 + 融合波形呈現（見第十二節）。
11. ~~結束畫面的融合波形目前只做「平滑過渡」，沒有「各 agent 波形飛向
    畫面中心再融合」的匯聚動畫效果~~ 已完成：`components/AgentMergeConverge.jsx`
    （見 10.3 節），採用環狀排列統一收斂的簡化版本，而不是量測真實
    DOM 位置的 FLIP 動畫（結束畫面掛載時原本的 AgentStage 方框已經
    卸載，量測不到「上一刻」的真實位置）。
12. 一般多 Agent 對話按下「結束對話」後，若後端當下正在跑某一輪的
    生成（`handle_user_text` 的 `async for` 尚未跑完），後端的
    WebSocket 收訊迴圈要等這一輪完全跑完才會真的處理 `end_session`
    （不像辯論模式有背景 `asyncio.Task` 可以插隊取消，見
    `routers/ws_voice_agents.py`）。目前前端已經處理「本地聲音立刻
    安靜下來、結束後陸續抵達的音訊不再播放」（`useVoiceAgentSession.js`
    的 `stopAllPlaybackImmediately()` + `sessionEndedRef`），但後端
    仍會繼續跑完目前這輪、白白耗用 LLM/TTS 資源才會真的送出
    `session_summary`；之後若要讓一般多 Agent 對話也能「立刻」取消
    生成，需要仿照辯論模式的背景 task 架構改寫該路由。

## 十二、內在法庭（VR 完整體驗流程：Unity 主持辯論 + 判決書紀念品）

對照《內在法庭_VR流程腳本.pdf》的展演流程，主系統由 Unity（openHCI_G2
專案，Meta Quest 2 via Link）擔任，網頁前端維持原功能不變。完整流程：

```
手機填 Big Five 問卷 + 錄「說說最近的煩惱」語音 + 選議題
  → 掃 Unity Start scene 傳票上的 QR 上傳（POST /link）
  → Unity 輪詢到 linked：旁白 → 就座 → 開庭演出（廣播帶入議題）
  → 兩位自動選出的「自我」agent 辯論（WS /ws/voice-debate，custom topic）
  → 法官敲法槌介入（限 3 次，語音插話走 user_intervene_audio）
  → 3 次用完該輪播完 / 回合上限 → 終結儀式 → session_summary（判決書）
  → Unity POST /result 回寫 + 顯示第二個 QR → 手機掃碼領取判決書
```

### 12.1 後端變更

- **議題（topic）**：`POST /api/onboarding-sessions/{id}/link` 新增選填
  `topic` form 欄位（手機端輸入/預設三選一）；留空時後端從聲音克隆樣本
  的 STT 逐字稿用 LLM 推導議題標題（`services/topic_derivation.py`，
  逾時/失敗 fallback 預設議題，mock LLM 環境直接截逐字稿前段，不呼叫
  假 LLM）。結果存進 `OnboardingSession.topic_title`，主系統載入後帶進
  `init_debate_session` 的 `topic_title`（`topic_id="custom"`，見第八節
  協定註記與 `agents/debate.py` 的 `build_custom_topic()`）。
- **判決書（verdict）**：`agents/debate.py` 新增 `generate_verdict()`，
  用 `agent_id="verdict"` 請 LLM 只輸出 JSON（欄位：`case_title` 案由、
  `initial_bias` 最初的成見、`viewpoint_a/b` 兩造主張、
  `judge_interventions` 法官介入意見、`final_verdict` 判決主文、
  `revised_belief` 修正後的信念、`closing_line` 結語）。解析有三層
  fallback（直接 parse → 剝 ```json fence → 抓第一個 {...}），完全失敗
  就把整段文字塞進主文退化呈現，絕不讓結束流程炸掉；`MockLLMService`
  對 `agent_id="verdict"` 回傳固定合法 JSON（`DEFAULT_VERDICT_REPLY`）。
  `ws_debate.py` 的 `end_session` 改送 `session_summary { text, verdict }`，
  verdict 生成失敗時 fallback 回舊的 `generate_summary()` 一句話。
  `OnboardingResult` 同步新增 `verdict` / `topic_title` 欄位。
- **QR 產生**：新增 `GET /api/qr?data=<內容>&size=<像素>`
  （`routers/qr.py`，依賴 `qrcode[pil]`，已加進 requirements/base.txt），
  供 Unity 的傳票 QR（手機連結頁網址）與領取 QR（手機結果頁網址）使用，
  Unity 端不需要內建 QR 編碼函式庫。

### 12.2 手機端變更（voice_clone_mobile）

- 步驟狀態機插入 `topic` 一步：`record → topic → connect`。錄音步驟的
  引導文案改成「說說你最近的煩惱」（一段錄音同時當克隆樣本與議題素材）；
  topic 步驟三選一：讓系統從錄音判斷（送空字串）/ 預設議題三選一
  （`PRESET_TOPICS`，純文字、統一走 custom topic 路徑）/ 自行輸入。
- `api/onboardingClient.js` 的 `linkOnboardingSession()` 新增 `topic`
  參數，隨 FormData 送出。
- `ResultPage.jsx` 新增 `VerdictDocument`：result 帶有 `verdict` 時以
  判決書排版呈現（案由/成見/兩造主張/介入意見/判決/修正後信念），
  沒有 verdict 的舊資料退回原本「一句總結語 + 融合波形」畫面。

### 12.3 Unity 端（openHCI_G2，摘要）

單場景（`Assets/tmp.unity`）三階段，`CourtExperienceDirector` 總導演：
Start scene（傳票 QR + 輪詢 + 旁白 + 引導光 + 就座偵測）→ 開庭演出
（頂燈依序點亮、廣播帶入議題、法槌規範說明）→ 自動從 5 位自我挑
Big Five 分數最偏離中性的 2 位（`SelfAgentSelector`）開始辯論 →
介入 3 次上限（`JudgeInterventionController.maxInterventions`）→
終結儀式（暗場 → 波形匯聚 → 計算進度條 → `VerdictPanel` 判決書 →
POST /result → 領取 QR → 退庭宣告）。廣播詞都預留 AudioClip 欄位，
沒音檔時以字幕+估算秒數代替。桌面測試熱鍵：Space（按住）模擬就座、
G 模擬敲法槌、E 強制結束辯論、R 重置整個體驗。
